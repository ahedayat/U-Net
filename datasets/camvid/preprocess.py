...
def generate_ann_mask(mask_path, labels):
    ...
    for ix, (label) in enumerate(labels):
        ...
        label_mask = (mask[:,:,0]==r)&(mask[:,:,1]==g)&(mask[:,:,2]==b)
        ann[ix, :, : ]= label_mask
    return torch.tensor(ann)


def preprocess( mode, items_name, camvid_path, annotation_version):
    assert mode in ['train', 'val', 'test'], 'preprocess mode must be "train" or "val" or "test".'
    ...
    mkdir('./{}'.format(mode), 'annotations_v{}'.format(annotation_version), forced_remove=False)

    labels = [line.split() for line in open('./labels_v{}.txt'.format(annotation_version))]
    for label in labels:
    ...
    for ix, (item_name) in enumerate(items_name):
        copy_file('{}/images'.format(camvid_path), item_name, './{}/images'.format(mode), item_name)
        
        ann = generate_ann_mask('{}/masks/{}'.format(camvid_path, item_name), labels)
        torch.save(ann, './{}/annotations_v{}/{}'.format(   mode, 
                                                            annotation_version,
                                                            os.path.splitext(item_name)[0]))

        write_file(mode, 'file_names', item_name, new_line=True, forced_remove_prev=False)
...
def _main():
    ...
    assert percent['train']+percent['val']+percent['test']==1, '"train percent"+ "val percent"+ "test percent" must be 1.'
    
    items_name = dict()
    items_name['train'] = images_name[ 0 : int(len(images_name) * percent['train']) ] 
    items_name['val'] = images_name[ int(len(images_name) * percent['train']) : int(len(images_name) * (percent['train'] + percent['val'])) ] 
    items_name['test'] = images_name[ int(len(images_name) * (percent['train'] + percent['val'])) : len(images_name) ] 
