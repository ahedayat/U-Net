...
def unet_train(unet,
    ...
    for epoch in range(start_epoch, start_epoch+num_epoch):
        ...
        for ix, (X, Y) in enumerate(data_loader):
            ...
            print('epoch: %d, batch: %d(x%d), prev loss:%.4d , curr loss: %.4d, D(loss)=%.4d, acc : %.2d %%' % (
                epoch,
                ix,
                batch_size,
                prev_loss,
        ...
        utility.save_np_file(
            '{}/train_accuracies/train_accuracies_epoch_{}'.format(report_path, epoch), accuracies)
        utility.save_np_file(
...
def unet_eval(unet,
    ...
    utility.mkdir(report_path, '{}_accuracies'.format(
        eval_mode), forced_remove=False)
    ...
    utility.save_np_file('{}/{}_accuracies/{}_accuracies{}'.format(report_path, eval_mode,
                                                                   eval_mode, '_epoch_{}'.format(epoch) if eval_mode == 'val' else ''), accuracies)
